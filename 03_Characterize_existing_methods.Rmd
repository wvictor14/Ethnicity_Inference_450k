...
g1 <- ggplot(pcs_cor %>% filter(Variable == 'Ethnicity'), 
       aes(x = PC, y = Rsquared, col = Cohort, group = Cohort)) +
  geom_point(aes(col = Cohort), alpha = 0.75, size = 0.9) + geom_line(size = 0.25) + theme_bw() +
  facet_wrap(~method, ncol = 1) + 
  scale_color_brewer(palette = 6, type = 'qual') +
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust=1),
        axis.title.y = element_text(angle = 0, vjust = 0.5)) +
  labs(y = expression(R^2), x = '') +
  scale_y_continuous(limits = c(0,1), breaks = c(0,0.5,1));g1  # ggsave h = 5, w = 3
...
g2 <- ggplot(pcs_cor %>% filter(grepl('Coordinate1', Variable)), 
       aes(x = PC, y = Rsquared, col = Cohort, group = Cohort)) +
  geom_point(aes(col = Cohort), alpha = 0.75, size = 0.9) + geom_line(size = 0.25) + theme_bw() +
  facet_wrap(~method, ncol = 1) + 
  ...
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust=1),
        axis.title.y = element_text(angle = 0, vjust = 0.5)) +
  labs(y = expression(R^2), x = '') +
  scale_y_continuous(limits = c(0,1), breaks = c(0,0.5,1));g2 # ggsave h = 5, w = 3
g3 <- ggplot(pcs_cor %>% filter(grepl('Coordinate2', Variable)), 
       aes(x = PC, y = Rsquared, col = Cohort, group = Cohort)) +
  geom_point(aes(col = Cohort), alpha = 0.75, size = 0.9) + geom_line(size = 0.25) + theme_bw() +
  facet_wrap(~method, ncol = 1) + 
  scale_colour_manual(values = c('#984ea3', '#ff7f00'))+
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust=1),
        axis.title.y = element_text(angle = 0, vjust = 0.5)) +
  labs(y = expression(R^2), x = '') +
  scale_y_continuous(limits = c(0,1), breaks = c(0,0.5,1)); g3 # ggsave h = 5, w = 3
