...
ggplot(cbind(pDat,pca$x) %>% mutate(Cohort_label = case_when(
  ...
  geom_point() + theme_bw() +
  labs( x = 'PC1 (18.59%)', y = 'PC2 (13.72%)', col = 'Cohort')
```
...
p2 <- ggplot(thresholds_plot %>% filter(thresholds != 1), 
             aes(x = thresholds, y = value, col = Class)) +
  geom_point(size = 1) + geom_line(size = 0.25) + #geom_vline(xintercept = 0.6, col = 'red') +
  labs(y = '', x = 'Ambiguous classification threshold', col = '') +
...
I decide on a threshold of 0.75, after playing around with a couple. This leads to only moderate
...
ggplot(cm_df_thresh) +
  ...
  scale_x_discrete(expand = c(0,0), 
                   labels = c('African (48)', 'Asian (35)', 'Caucasian (384)', 
                              'Ambiguous (32)')) + 
  scale_y_discrete(expand=c(0,0),
                   labels = c('African (57)', 'Asian (35)', 'Caucasian (389)'))+
  labs(fill = 'Samples in\nself-reported\nclass (%)\n', 
       x = 'Predicted Ethnicity', y = 'Self-reported\nEthnicity') +
  coord_equal()
```
